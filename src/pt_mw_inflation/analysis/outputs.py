...
def _interval_macros(ordered: pd.DataFrame, row: int, *, prefix: str) -> dict[str, str]:
    """Quantities describing the inverted interval, for the prose to cite.

    Emitted only where the estimator produced intervals, so that a manuscript
    citing them fails to build rather than quietly reporting a stale number from
    a run that did not compute them.

    Args:
        ordered: Horizon estimates with a reset index.
        row: Position of the horizon the peak macros describe.
        prefix: Distinguishes one design's macros from another's.

    Returns:
        Macro names and values, empty when no interval was estimated.
    """
    if "interval_lower" not in ordered.columns:
        return {}

    lower = ordered["interval_lower"].to_numpy(dtype=float)
    upper = ordered["interval_upper"].to_numpy(dtype=float)
    widths = upper - lower
    widest = int(np.nanargmax(widths))

    return {
        f"{prefix}PeakIntervalLower": f"{lower[row]:.2f}",
        f"{prefix}PeakIntervalUpper": f"{upper[row]:.2f}",
        f"{prefix}WidestIntervalLower": f"{lower[widest]:.2f}",
        f"{prefix}WidestIntervalUpper": f"{upper[widest]:.2f}",
        f"{prefix}WidestIntervalHorizon": f"{int(ordered['horizon'].to_numpy()[widest])}",
        # Whether every interval closed inside the search. The manuscript reads
        # these as bounds, and that reading is only honest where the search found
        # both ends rather than running out of room.
        f"{prefix}IntervalsBounded": (
            "all" if bool(ordered["interval_bounded"].all()) else "not all"
        ),
    }


def _format_interval(row: pd.Series) -> str:
    """Render one inverted interval, marking a search that did not close.

    An interval whose search was exhausted is reported with its ends and a
    marker rather than silently, because those ends are where we stopped looking
    and not where the interval stops. Printing them bare would turn a failure to
    bound the effect into an apparent bound on it.
    """
    lower, upper = float(row["interval_lower"]), float(row["interval_upper"])
    if not (np.isfinite(lower) and np.isfinite(upper)):
        return "---"
    rendered = f"$[{lower:.2f},\\, {upper:.2f}]$"
    return rendered if bool(row["interval_bounded"]) else rendered + r"$^{\ast}$"
...
def write_regional_design_table(
    ...
    The inverted interval is carried beside the p-values wherever the estimator
    produced one. A null p-value says that zero survives the test and nothing
    else; the interval says what else survives it, which is the difference
    between a design that found no effect and one that could not have found one.

    Args:
        estimates: Horizon estimates from the local-projection estimator.
        path: Destination `.tex` file.
        command: Command that regenerates the table, named in its header.
    ...
    with_interval = "interval_lower" in estimates.columns
    header = r"Horizon (months) & Coefficient & Clustered $p$ & Bootstrap $p$ & Holm $p$"
    lines = [
        "% Generated by pt_mw_inflation.analysis.outputs.write_regional_design_table.",
        f"% Do not edit: regenerate with '{command}'.",
        r"\begin{tabular}{lrrrr" + ("r" if with_interval else "") + "}",
        r"\toprule",
        (header + r" & 95\% interval" if with_interval else header) + r" \\",
        r"\midrule",
    ]
...
def write_identification_macros(
    ...
    values = {
        ...
        # What the design could not rule out, at the horizon it speaks most
        # loudly about. The peak coefficient on its own invites the reader to
        # treat it as an estimate of something; its interval is what says how
        # much of the range it leaves open.
        **_interval_macros(ordered, peak_row, prefix=""),
...
def write_exposure_design_macros(estimates: pd.DataFrame, path: Path) -> Path:
    ...
    ordered = estimates.reset_index(drop=True)
    widest = int(ordered["coefficient"].abs().to_numpy().argmax())
    values = {
        ...
        "ExposureWidestCoefficient": f"{ordered['coefficient'].to_numpy()[widest]:.2f}",
        "ExposureWidestHorizon": f"{int(ordered['horizon'].to_numpy()[widest])}",
        # This design rejects nothing, and a table of nulls cannot say whether
        # that is because there is no effect or because nothing could have been
        # detected. The interval is the only thing here that distinguishes them.
        **_interval_macros(ordered, widest, prefix="Exposure"),
    }
...
#: Statutory rise the implied responses are quoted for, matching the scale
#: :func:`~pt_mw_inflation.analysis.inference.detectable_effects` uses.
_STATUTORY_RISE = 0.10


def write_structural_design_macros(
    estimates: pd.DataFrame,
    path: Path,
    *,
    identifying_spread: float | None = None,
    cost_share_ceiling: float | None = None,
) -> Path:
    """Emit what the region-by-category design supports, as generated quantities.

    Reported like the other two designs, and with the same discipline: the
    interval rather than the p-value carries the finding, because a table of
    nulls cannot distinguish a design that found no effect from one that could
    not have found any.

    The coefficient itself is not interpretable and is deliberately not reported
    as though it were. The regressor weights each industry by the region's
    employment share in it, which is what gives the measure its regional
    dimension and also means it is not a cost share but a fraction of one. A
    coefficient of one is therefore *not* complete pass-through, and there is no
    fixed factor that would make it so: the fraction runs from under a hundredth
    to two fifths across cells.

    What is interpretable is the implied differential price response in points,
    which is the coefficient scaled by the identifying spread and a stated
    statutory rise, and which can be compared against a ceiling this design
    supplies. The largest national minimum-wage cost share of any consumption
    category bounds what complete pass-through could produce: if every euro of
    minimum-wage cost reached prices, a category with that share could move by
    at most the share times the rise. An implied response above that ceiling is
    not a large estimate, it is an impossible one, and both numbers are emitted
    so the comparison is in the paper rather than in the reader's head.

    Args:
        estimates: Horizon estimates from the region-by-category design.
        path: Destination `.tex` file.
        identifying_spread: Range of the double-demeaned exposure matrix, in
            points. This is what the design has after its fixed effects, and the
            raw spread of the measure would overstate it.
        cost_share_ceiling: Largest national minimum-wage cost share of any
            consumption category, as a proportion. Supplied, the implied
            responses and the ceiling they must respect are both emitted.
    ...
    ordered = estimates.reset_index(drop=True)
    peak = int(ordered["coefficient"].abs().to_numpy().argmax())
    values = {
        "StructuralHorizons": f"{len(ordered)}",
        "StructuralClusteredSignificant": f"{int((ordered['p_value_clustered'] < 0.05).sum())}",
        "StructuralBootstrapSignificant": f"{int((ordered['p_value_bootstrap'] < 0.05).sum())}",
        "StructuralHolmSignificant": f"{int((ordered['p_value_bootstrap_holm'] < 0.05).sum())}",
        "StructuralMinBootstrapP": f"{ordered['p_value_bootstrap'].min():.3f}",
        "StructuralPeakCoefficient": f"{ordered['coefficient'].to_numpy()[peak]:.2f}",
        "StructuralPeakHorizon": f"{int(ordered['horizon'].to_numpy()[peak])}",
        "StructuralClusters": f"{int(ordered['clusters'].max())}",
        "StructuralCells": f"{int(ordered['observations'].max())}",
        **_interval_macros(ordered, peak, prefix="Structural"),
    }
    if identifying_spread is not None:
        values["StructuralIdentifyingSpread"] = f"{identifying_spread:.2f}"

    if identifying_spread is not None and cost_share_ceiling is not None:
        # The coefficient scaled into the units the ceiling is expressed in:
        # points of differential price response across the identifying range,
        # for a statutory rise of `_STATUTORY_RISE`.
        implied = (
            ordered["coefficient"].abs() * (identifying_spread / 100.0) * _STATUTORY_RISE * 100.0
        )
        ceiling = 100.0 * cost_share_ceiling * _STATUTORY_RISE
        values |= {
            "StructuralRisePct": f"{100 * _STATUTORY_RISE:.0f}",
            "StructuralPeakResponsePP": f"{implied.to_numpy()[peak]:.1f}",
            "StructuralPassThroughCeilingPP": f"{ceiling:.1f}",
            "StructuralCostShareCeilingPct": f"{100 * cost_share_ceiling:.0f}",
            "StructuralAboveCeiling": f"{int((implied > ceiling).sum())}",
        }

    lines = [
        "% Generated by pt_mw_inflation.analysis.outputs.write_structural_design_macros.",
        "% Do not edit: regenerate with 'ptmw analyse structural-design'.",
    ]
    lines += [rf"\providecommand{{\{name}}}{{{value}}}" for name, value in values.items()]
    lines.append("")
