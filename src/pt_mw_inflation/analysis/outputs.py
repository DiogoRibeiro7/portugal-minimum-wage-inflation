...
def write_pre_trend_macros(result: JointTest, path: Path) -> Path:
    ...
    values = {
        "PreTrendLeads": f"{result.restrictions}",
        "PreTrendWald": f"{result.statistic:.0f}",
        "PreTrendP": f"{result.p_value:.3f}",
        "PreTrendRejects": "rejects" if result.p_value < 0.05 else "does not reject",
    }
